...
const TemplateSelector = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] flex flex-col items-center p-8 py-16 overflow-y-auto">
      <div className="max-w-[1240px] w-full">
        ...
        >
          <div>
            <h1 className="text-5xl font-black text-slate-900 tracking-tighter mb-3">Resume Gallery</h1>
            <p className="text-[14px] text-slate-400 font-bold uppercase tracking-[0.3em] ml-1">Select your design foundation</p>
          </div>
          <Link to="/dashboard" className="px-8 py-4 bg-white border border-slate-200 text-slate-900 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-50 transition-all text-[12px] shadow-sm flex items-center gap-2 group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-10">
          {templates.map((t, idx) => (
            ...
            >
              <div className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-[0_15px_45px_rgb(0,0,0,0.03)] group-hover:shadow-[0_40px_80px_rgb(0,0,0,0.12)] group-hover:border-[#0ea5e9]/30 transition-all duration-700">
                {/* Visual Preview - Focused & Zoomed */}
                <div className="h-[300px] bg-[#fdfdfd] relative overflow-hidden flex justify-center border-b border-slate-100">
                  {idx === 0 && (
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-[#0ea5e9]/5 transition-all duration-700 z-10 flex items-center justify-center opacity-0 group-hover:opacity-100 backdrop-blur-[2px]">
                     <div className="bg-white px-8 py-4 rounded-2xl shadow-2xl scale-75 group-hover:scale-100 transition-all duration-500">
                ...
                <div className="p-10 pt-12 text-center relative">
                   {/* Background Number */}
                   <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-[80px] font-black text-slate-100 select-none pointer-events-none opacity-40 italic">
                     {idx + 1}
                   </span>
                   
                   <h3 className="text-xl font-black text-slate-900 mb-2 relative z-10">{t.name}</h3>
                   <p className="text-[13px] text-slate-400 font-bold leading-relaxed relative z-10">
                     {t.description}
                   </p>
                </div>
              </div>
